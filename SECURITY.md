...
- **The engine is never exposed directly.** codebase-memory-mcp has no
...
Vulnerabilities in codebase-memory-mcp itself belong in [its own security
process](https://github.com/DeusData/codebase-memory-mcp/security). If a CBM
issue is exploitable specifically because of how repo-mcp deploys it, report
it here as well — that part is ours.
