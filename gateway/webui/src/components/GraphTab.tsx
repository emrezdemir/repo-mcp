...
import { repoInfo as fetchRepoInfo } from "../api/platform";
...
export function GraphTab({ project }: GraphTabProps) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    // list_projects already carries the git metadata, so there is no second
    // endpoint here and nothing extra to authorize.
    fetchRepoInfo(project)
      .then((info) => {
        if (!cancelled && info) setRepoInfo(info as unknown as RepoInfo);
      })
      .catch(() => {});
    return () => {
      cancelled = true;
