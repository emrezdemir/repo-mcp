...
export async function fetchLayout(
  ...
  // The layout is served by this squad's engine and proxied by the gateway,
  // which authorizes it exactly as it authorizes a tool call — so it needs the
  // same token and squad headers every other request carries.
  const res = await fetch(`/api/layout?${params}`, { headers: await authHeaders() });
